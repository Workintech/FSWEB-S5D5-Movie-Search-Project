...
test("DisplayMovieInfo component'i div.movieInfo oluşturuyor.", async () => {
  const element = container.querySelector('.movie-container > div#movieInfo');
  expect(element).toBeInTheDocument();
});

test("div.movieInfo içinde doğru film başlığı ile  h2 tag'i oluşturuluyor.", async () => {
...
test("Year bilgisi bir span içinde ilk paragraf tag'inde kullanılmış.", async () => {
...
test("Year bilgisi doğru metin ile -arada bir boşluk bırakılarak(örn: Year: 1977)- ilk paragraf'a yerleştirilmiş.", async () => {
  const element = container.querySelectorAll(
...
test("Genre bilgisi bir span içinde ikinci paragraf tag'inde kullanılmış.", async () => {
  const element = container.querySelectorAll(
    '.movie-container > div#movieInfo > p > span'
  );
...
test("Genre bilgisi doğru metin ile -arada bir boşluk bırakılarak- ikinci paragraf'a yerleştirilmiş.", async () => {
  const element = container.querySelectorAll(
    '.movie-container > div#movieInfo > p'
  );
...
test("Plot bilgisi bir span içinde üçüncü paragraf tag'inde kullanılmış.", async () => {
  const element = container.querySelectorAll(
    '.movie-container > div#movieInfo > p > span'
  );
...
test("Plot bilgisi doğru metin ile -arada bir boşluk bırakılarak- üçüncü paragraf'a yerleştirilmiş.", async () => {
  const element = container.querySelectorAll(
    '.movie-container > div#movieInfo > p'
  );
...
test("Film posterine alt altribute'u doğru metin ile eklenmiş.(Film başlığın sonundaki Poster yazısına dikkat)", async () => {
  const element = container.querySelector(
    '.movie-container > div#movieInfo > img:nth-child(5)'
  );
...
test("DisplayMovieInfo component'i Film bulunamadığında ekranda doğru metni içeren paragraf elementi dönüyor ", async () => {
